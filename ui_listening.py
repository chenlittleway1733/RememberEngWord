...
def render_listening_page(merged_df: pd.DataFrame, user_id: str, user_name: str, filter_state: dict):
    ...
    # iPad Safari / Streamlit Cloud 對自動播放限制較多。
    # 為避免換下一題時殘留上一題音檔，本版取消自動播放，
    # 改成每題由使用者按下按鈕播放目前題目的音檔。
    st.info("請按下方按鈕播放本題音檔。")
    audio_button(correct_answer, "🔊 播放本題音檔", key=f"listening_replay_{user_id}_{row.get('word_id','')}_{st.session_state.listening_count_done}")
